...
if ( ! function_exists('jvp_send_wawp_message') ) {
  function jvp_send_wawp_message($to, $message){
    ...
    $inst = trim(get_option('jvp_wawp_instance_id',''));
    $tok  = trim(get_option('jvp_wawp_access_token',''));
    if (!$api || !$inst || !$tok) return new WP_Error('wawp_settings','إعدادات Wawp غير مكتملة.');

    $payload = ['instance_id'=>$inst,'access_token'=>$tok,'to'=>$to,'message'=>wp_strip_all_tags($message)];
    $r = wp_remote_post($api,['timeout'=>30,'headers'=>['Content-Type'=>'application/json; charset=utf-8'],'body'=>wp_json_encode($payload)]);
    if (!is_wp_error($r)) {
      $c = wp_remote_retrieve_response_code($r);
      if ($c>=200 && $c<300) return true;
      error_log('[JVP][Wawp][JSON] HTTP '.$c.' BODY: '.substr(wp_remote_retrieve_body($r),0,300));
    } else { error_log('[JVP][Wawp][JSON] '.$r->get_error_message()); }
    $r2 = wp_remote_post($api,['timeout'=>30,'body'=>$payload]);
    if (is_wp_error($r2)) { error_log('[JVP][Wawp][FORM] '.$r2->get_error_message()); return $r2; }
    $c2 = wp_remote_retrieve_response_code($r2);
    if ($c2>=200 && $c2<300) return true;
    return new WP_Error('wawp_http','Wawp HTTP '.$c2.': '.substr(wp_remote_retrieve_body($r2),0,200));
  }
}
