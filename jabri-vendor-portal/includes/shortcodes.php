...
if ( ! function_exists('jvp_normalize_msisdn') ) {
    function jvp_normalize_msisdn( $raw, $default_cc = '+962' ) {
        ...
        // If starts with + already, assume ok
        if ( strpos($s, '+') === 0 ) return $s;
        // If local leading zero, remove it and prepend default CC
        $s = ltrim($s, '0');
        return $default_cc . $s;
    }
}
...
function jvp_brand_css() {
    ...
    $c_txt = '#222';
    return '<style>
    .jvp-wrap{direction:rtl;font-family:Tahoma,Arial,sans-serif;background:'.$c_bg.';padding:18px;border-radius:16px;max-width:680px;margin:10px auto;}
    .jvp-card{background:#fff;border:1px solid #eee;border-radius:16px;padding:18px;box-shadow:0 6px 16px rgba(0,0,0,.06)}
    .jvp-title{margin:0 0 12px;color:'.$c_p3.';font-size:20px;font-weight:700}
    .jvp-row{display:flex;gap:12px;flex-wrap:wrap}
    .jvp-row .jvp-col{flex:1 1 240px}
    .jvp-label{display:block;margin-bottom:6px;color:'.$c_txt.';font-weight:600}
    .jvp-input, .jvp-file{width:100%;padding:10px 12px;border:1px solid #ddd;border-radius:12px}
    .jvp-btn{display:inline-block;padding:10px 16px;border-radius:12px;background:'.$c_p1.';color:#fff;text-decoration:none;border:none;cursor:pointer}
    .jvp-btn:hover{background:'.$c_p2.'}
    .jvp-linkbtn{display:inline-block;padding:10px 16px;border-radius:12px;background:#fff;border:1px solid '.$c_p1.';color:'.$c_p1.';text-decoration:none}
    .jvp-linkbtn:hover{color:#fff;background:'.$c_p1.'}
    .jvp-muted{color:#666;font-size:13px}
    .jvp-note{background:#fff3cd;border:1px solid #ffeeba;color:#856404;border-radius:12px;padding:10px 12px;margin:6px 0}
    .jvp-success{background:#d4edda;border:1px solid #c3e6cb;color:#155724;border-radius:12px;padding:10px 12px;margin:6px 0}
    .jvp-error, .woocommerce-error{background:#f8d7da;border:1px solid #f5c6cb;color:#721c24;border-radius:12px;padding:10px 12px;margin:6px 0}
    .jvp-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:10px}
    </style>';
}
...
add_shortcode('vendor_registration_form', function($atts = [], $content = ''){
    ...
    if ( 'send_otp' === $action && isset($_POST['jvp_phone']) && isset($_POST['jvp_nonce']) && wp_verify_nonce($_POST['jvp_nonce'], 'jvp_reg') ) {
        ...
        $phone = jvp_normalize_msisdn($_POST['jvp_phone'] ?? '', $default_cc);
        if ( empty($phone) ) {
...
add_shortcode('vendor_login_form', function($atts = [], $content = ''){
    if ( is_user_logged_in() ) {
        $url = function_exists('wc_get_account_endpoint_url') ? wc_get_account_endpoint_url('orders') : home_url('/my-account/orders/');
        ...
        if ( empty($identity) || empty($password) ) {
            ...
            if ( preg_match('/^[+0-9][0-9\s\-()+]*$/', $identity) ) {
                ...
                if ( ! empty($u) && $u[0] instanceof WP_User ) {
                    ...
                    if ( username_exists($candidate) ) $user_login = $candidate;
                }
            }
            ...
            if ( is_wp_error($user) ) {
                ...
                $target = function_exists('wc_get_account_endpoint_url') ? wc_get_account_endpoint_url('orders') : home_url('/my-account/orders/');
